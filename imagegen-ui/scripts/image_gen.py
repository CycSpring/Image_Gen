...
ALLOWED_SIZES = {"1024x1024", "1536x1024", "1024x1536", "2048x2048", "2048x1152", "1152x2048", "auto"}
...
def _validate_size(size: str) -> None:
    if size not in ALLOWED_SIZES:
        _die(
            "size must be one of 1024x1024, 1536x1024, 1024x1536, 2048x2048, 2048x1152, 1152x2048, or auto for GPT image models."
        )
...
def _create_async_client():
    ...
    return AsyncOpenAI(timeout=300.0)
...
def main() -> int:
    ...
    try:
        ...
        is_network_issue = any(
            x in err_name.lower() or x in err_msg.lower()
            for x in ["timeout", "timed out", "connect", "connection", "dns"]
        )
        if is_network_issue:
            base_url = os.getenv("OPENAI_BASE_URL", "")
            if not base_url or "api.openai.com" in base_url:
                _die(
                    "网络连接或请求超时。如果您在中国大陆网络环境下使用，请在客户端顶部配置有效的国内中转 API Base URL（例如：https://api.openai-hk.com/v1 等）或确保本地代理网络顺畅。"
                )
            else:
                _die(f"网络连接或请求超时 (Base URL: {base_url})。详情: {e}")
        _die(f"任务执行失败: {e}")
    return 0
